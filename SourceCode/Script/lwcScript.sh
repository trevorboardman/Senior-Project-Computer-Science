...
#Authors: Trevor Boardman & Joseph Kadrich
...
#If you have any problems, please contact either Trevor Boardman or Joseph Kadrich
#Trevor Boardman - 614-716-8871 - Boardmantm6147@gmail.com
#Joseph Kadrich - kadrichjp@mail.mvnu.edu

#If you are planning to implement this script and it's affiliated components, contact Trevor Boardman and/or Joseph Kadrich before attempting to do so.
...
play -q /home/pi/LWC/startup.wav
...
do
	...
	do
		read tempInput
		CounselorID1=$(ssh boardmantm@206.21.202.62 "mysql -N -u $dbU -p$dbP LWC -e 'SELECT CounselorID FROM Counselor WHERE CounselorID='$tempInput';'")
		
		if [ -z "$CounselorID1" ] ; then
			CounselorID1=0
		fi

		ClientID1=$(ssh boardmantm@206.21.202.62 "mysql -N -u $dbU -p$dbP LWC -e 'SELECT ClientID FROM Client WHERE ClientID='$tempInput';'")
		...
		elif [ "$ClientIDFound" = true ] && [ "$tempInput" -eq "$tempClientID" ]  ; then
			#cardCount=$((cardCount-1))
			play -q /home/pi/LWC/already.wav #Change to file path of your sound files
			continue
	...
	if [ "$room1Session" = true ] && [ "$doorID" -eq "$room1" ] ; then
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration1\" WHERE sessionID = $room1SessionID;'"
	...
	#Room 2 Session End	
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration2\" WHERE sessionID = $room2SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room2Session=false
	...
	#Room 3 Session End	
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration3\" WHERE sessionID = $room3SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room3Session=false	
	...
	#Room 4 Session End
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration4\" WHERE sessionID = $room4SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room4Session=false	
	...
	#Room 5 Session End
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration5\" WHERE sessionID = $room5SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room5Session=false	
	...
	#Room 6 Session End
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration6\" WHERE sessionID = $room6SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room6Session=false	
	...
	#Room 7 Session End
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration7\" WHERE sessionID = $room7SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room7Session=false	
	...
	#Room 8 Session End
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'UPDATE CounselingLog SET SessionEnd = \"$sessionEndTime\", Duration = \"$duration8\" WHERE sessionID = $room8SessionID;'"
		play -q /home/pi/LWC/ended.wav #Change to file path of your sound files
		#Sets current session to false ending the session
		room8Session=false	
	...
	#Checks if there is no current session in the specified room
		...
		ssh boardmantm@206.21.202.62 "mysql -u $dbU -p$dbP LWC -e 'INSERT INTO CounselingLog (sessionID, Room, CounselorID, ClientID, SessionStart, Date) VALUES ($sessionID,\"$doorID\",\"$CounselorID\",\"$ClientID\",\"$sessionStartTime\",\"$sessionDate\");'"
